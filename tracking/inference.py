...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        if (sumOfValues := self.total()) > 0:
            self.update((key, value / sumOfValues) for key, value in self.items())
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        predictBeliefs = DiscreteDistribution()
        for oldPos in self.allPositions:
            if self.beliefs[oldPos] == 0:
                continue
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for p in newPosDist:
                predictBeliefs[p] += self.beliefs[oldPos] * newPosDist[p]
        self.beliefs = predictBeliefs
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        for p in itertools.cycle(self.legalPositions):
